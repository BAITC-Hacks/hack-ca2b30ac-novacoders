...
export default function SummaryCards({ summary }: { summary: RunSummary }) {
  return <dl className="summary-strip" aria-label="Итоги расчёта">
    ...
    <div><dt>Объём заказа</dt><dd>{formatNumber(summary.totalUnits)} <span>ед.</span></dd></div>
    <div className="summary-cost"><dt>Ориентировочная стоимость</dt><dd>{formatMoney(summary.estimatedCost)}</dd></div>
  </dl>
}
