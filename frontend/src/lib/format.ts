import type { Decision, RecommendationItem, Urgency } from '../types/api'
...
export const urgencyLabels: Record<Urgency, string> = { CRITICAL: 'Критично', HIGH: 'Высокая', MEDIUM: 'Средняя', LOW: 'Низкая' }
export const sortItems = (items: RecommendationItem[]) => [...items].sort((a, b) => {
  const rank = (item: RecommendationItem) => item.decision === 'REVIEW' ? 0 : item.decision === 'BUY' ? item.urgency === 'CRITICAL' ? 1 : 2 : 3
  return rank(a) - rank(b) || a.name.localeCompare(b.name, 'ru')
})
export function quantityError(value: string): string | null {
  if (!/^\d+$/.test(value) || !Number.isSafeInteger(Number(value))) return 'Введите целое неотрицательное число.'
  return null
}
